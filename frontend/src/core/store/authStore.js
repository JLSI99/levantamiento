class AuthStore {
  constructor() {
    ...
    this.state = {
      ...
      roles: this._safeParse(localStorage.getItem('tx_inv_roles')) || [],
  ...
  setSession(accessToken, refreshToken, user, capabilities, roles = []) {
    this.state = {
    ...
    localStorage.setItem('tx_inv_roles', JSON.stringify(roles || []));
  ...
  clearSession() {
    ...
    localStorage.removeItem('tx_inv_roles');
    localStorage.removeItem('tx_inv_caps');
    this.emit();
  }

  _handleStorageChange(event) {
    const keysToSync = ['tx_inv_token', 'tx_inv_refresh', 'tx_inv_user', 'tx_inv_roles', 'tx_inv_caps'];
