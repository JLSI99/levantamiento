...
@router.get("/tipos-bien/{id_tipo}", response_model=schemas.TipoBienOut)
@limiter.limit("30/minute")
...
@router.patch("/tipos-bien/{id_tipo}", response_model=schemas.TipoBienOut)
@limiter.limit("30/minute")
async def actualizar_tipo_bien(
    ...
    if not tipo.esta_activo:
        raise HTTPException(status_code=400, detail="No puedes editar un tipo de bien inactivo.")
...
async def borrar_tipo_bien(
    ...
    if not tipo:
        raise HTTPException(status_code=404, detail="Tipo de bien no encontrado.")
    if not tipo.esta_activo:
        raise HTTPException(status_code=400, detail="El tipo de bien ya está dado de baja.")
...
async def crear_bien(
    ...
    if any(not tipo.esta_activo for tipo in tipos):
         raise HTTPException(status_code=400, detail="No puedes asignar un tipo de bien inactivo a un activo nuevo.")
...
@router.get("/bienes/{id_bien}", response_model=schemas.BienOut)
@limiter.limit("30/minute")
...
@router.patch("/bienes/{id_bien}", response_model=schemas.BienOut)
@limiter.limit("30/minute")
async def actualizar_bien(
    ...
    if not bien.esta_activo:
        raise HTTPException(status_code=400, detail="No puedes editar un bien inactivo.")
...
@router.delete("/bienes/{id_bien}", status_code=status.HTTP_204_NO_CONTENT)
@limiter.limit("10/minute")
async def borrar_bien(
    ...
    if not bien:
        raise HTTPException(status_code=404, detail="Bien no encontrado.")
    if not bien.esta_activo:
        raise HTTPException(status_code=400, detail="El bien ya está dado de baja.")
