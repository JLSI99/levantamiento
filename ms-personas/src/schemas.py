...
PATRON_NOMBRES_ESPANOL = r"^[a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s]+$"
...
class PersonaCreate(PersonaBase):
    ...
    def limpiar_y_validar_nombres(cls, v: str) -> str:
        v = v.strip()
        if not re.match(PATRON_NOMBRES_ESPANOL, v):
...
class PersonaUpdate(BaseModel):
    ...
    def limpiar_y_validar_nombres(cls, v: Optional[str]) -> Optional[str]:
        ...
        v = v.strip()
        if not re.match(PATRON_NOMBRES_ESPANOL, v):
            raise ValueError('Los nombres y apellidos solo pueden contener letras y caracteres válidos en español')
        return v
    ...
    def validar_formato_curp(cls, v: Optional[str]) -> Optional[str]:
        ...
        v = v.upper().strip()
        if not re.match(PATRON_CURP_RENAPO, v):
            raise ValueError('El formato del CURP no cumple con el estándar oficial de RENAPO')
        return v
