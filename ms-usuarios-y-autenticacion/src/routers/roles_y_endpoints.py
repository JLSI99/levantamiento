...
from sqlalchemy.orm import selectinload # NUEVO: Importación necesaria para evitar errores asíncronos en M2M
...
async def list_all_permisos(db: AsyncSession = Depends(get_db)):
...
async def get_permisos_by_rol(
    ...
    if not permisos:
        raise HTTPException(status_code=404, detail="El rol no existe o no tiene permisos asignados")
...
async def revoke_permiso_from_rol(
    ...
    if permiso not in rol.permisos:
        raise HTTPException(status_code=400, detail="El permiso no está asignado a este rol")
